...
// Configuration setup 
const setups_config_server_1 = require("./setups.config.server");
// controller imports
// Routes imports
const storage_route_server_1 = require("../routes/storage.route.server");
class App {
    constructor() {
    ...
    config() {
        ...
        this.app.use(function (req, resp, next) {
            ...
            resp.setHeader('Access-Control-Allow-Headers', 'application/json, X-Requested-With,application/x-www-form-urlencoded, image/png, Content-Type, Authorization, Origin, Accept, multipart/form-data , application/octet-stream');
    ...
    routes() {
        new storage_route_server_1.default(this.app);
        console.log('routes initialized');
        // initialise and configure routes against app instance
    }
    init() {
        setups_config_server_1.default.init((err) => {
            // if (err) {
